...
namespace axq
{
	AutoChesser::AutoChesser(const std::string& settingFileName, EngineType engineType)
    ...
    AXQResult AutoChesser::AutoPlayChess()
    {
		...
		if (m_Engine == nullptr)
		{
			if (m_EngineType == EngineType::PIKAFISH)
				m_Engine = new Pikafish("pikafish", Pikafish::m_Exe);
			else
				m_Engine = new Growfish("growfish", Growfish::m_Exe);
			m_Engine->InitEngine();
	...
	AXQResult AutoChesser::MovePiece(RunType runType, std::string &decidedMove)
	{
		...
		if (m_EngineType == EngineType::GROWFISH)
		{
